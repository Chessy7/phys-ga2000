...
#e
frequency = 2*np.pi/((np.max(time)-np.min(time))/2)
Ao = np.zeros((len(time), 9))
Ao[:, 0] = 1.
for i in np.arange(1,9,2):
    Ao[:, i] = np.sin(frequency*time*(i+1)/2)
    Ao[:, i+1] = np.cos(frequency*time*(i+1)/2)
(uo, wo, vot) = np.linalg.svd(Ao, full_matrices=False)
print(np.max(w)/np.min(w))
Oinv = vot.transpose().dot(np.diag(1. / wo)).dot(uo.transpose())
co = Oinv.dot(signal)
yo = Ao.dot(co) 
plt.plot(time, signal,'.', label='data')
plt.plot(time, yo, '.', label='model')
plt.xlabel("time")
plt.ylabel("signal")
plt.legend()
